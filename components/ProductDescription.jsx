    ...
    return (
        <div className="my-18 text-sm text-slate-600">
                    ...
                    {Array.isArray(product.rating) && product.rating.length > 0 ? (
                        product.rating.map((item,index) => (
                            <div key={index} className="flex gap-5 mb-10">
                                <Image src={item.user?.image || '/placeholder-store-logo.svg'} alt="" className="size-10 rounded-full" width={100} height={100} />
                                <div>
            ...
            <div className="flex gap-3 mt-14">
                <Image src={product.store?.logo || '/placeholder-store-logo.svg'} alt="" className="size-11 rounded-full ring ring-slate-400" width={100} height={100} />
                <div>
                    <p className="font-medium text-slate-600">Product by {product.store?.name || 'Our Store'}</p>
                    {product.store?.username ? (
                        ...
                        <span className="flex items-center gap-1.5 text-slate-400"> store info unavailable </span>
                    )}
                </div>
            </div>
        </div>
