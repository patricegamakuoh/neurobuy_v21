...
export default async function VendorStorefront({ params }: { params: { vendorId: string } }) {
	const products = await listProducts({});
	const vendorProducts = products.filter((p) => p.vendor_id === params.vendorId);
	return (
		<div className="max-w-5xl mx-auto w-full p-5">
			<h1 className="text-2xl font-semibold mb-6">Vendor Store</h1>
			{vendorProducts.length === 0 ? (
				<p className="text-sm text-muted-foreground">No products for this vendor.</p>
			) : (
